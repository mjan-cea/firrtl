...
class FirrtlStageSpec extends FeatureSpec with GivenWhenThen with Matchers with firrtlTests.Utils
  ...
  info("I migrate from Driver to FirrtlStage")
  feature("FirrtlStage migration helpers") {
    ...
    Seq(
      ...
      FirrtlStageTest(args    = Array("--top-name", "foo"),
                      circuit = None,
                      stdout  = optionRemoved("--top-name/-tn"),
                      result  = 1),
      FirrtlStageTest(args    = Array("-tn"),
                      circuit = None,
                      stdout  = optionRemoved("--top-name/-tn"),
                      result  = 1),
      /* Removed --split-modules/-fsm handling */
      FirrtlStageTest(args    = Array("--split-modules"),
                      circuit = None,
                      stdout  = optionRemoved("--split-modules/-fsm"),
                      result  = 1),
      FirrtlStageTest(args    = Array("-fsm"),
                      circuit = None,
                      stdout  = optionRemoved("--split-modules/-fsm"),
                      result  = 1),
      /* User doesn't specify any emitter */
      FirrtlStageTest(args    = Array("-X", "none"),
                      stdout  = Some("No emitter specified."),
                      result  = 0)
    )
      .foreach(runStageExpectFiles)
  }
