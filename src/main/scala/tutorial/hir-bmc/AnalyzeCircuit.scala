...
class Ledger {
  ...
  private val moduleDefWire = mutable.Map[String, Int]()
  private val moduleDefRegister = mutable.Map[String, Int]()    
  ...
  def foundDefWire(): Unit = moduleName match {
    case None => sys.error("Module name not defined in Ledger!")
    case Some(name) => moduleDefWire(name) = moduleDefWire.getOrElse(name, 0) + 1
  }  
  def foundDefRegister(): Unit = moduleName match {
    case None => sys.error("Module name not defined in Ledger!")
    case Some(name) => moduleDefRegister(name) = moduleDefRegister.getOrElse(name, 0) + 1
  }  
  ...
  def serialize: String = {
    modules map { myName =>
      s"$myName => ${moduleMuxMap.getOrElse(myName, 0)} muxes! and ${moduleDefWire.getOrElse(myName, 0)} DefWire and ${moduleDefRegister.getOrElse(myName, 0)} DefRegister!"
    } mkString "\n"
  }
}
...
class AnalyzeCircuit extends Transform {
  ...
  def walkModule(ledger: Ledger)(m: DefModule): DefModule = {
    ...
    m match {
      case Module(info,name,ports,body) =>
        println(s"MODULE $name, INFO $info, PORTS $ports\n")
      case other => other
    }
  ...
  def walkStatement(ledger: Ledger)(s: Statement): Statement = {
     ...
     */
    val visited = s map walkExpression(ledger)
    visited match {
      case DefWire(info, name, tpe) =>
        ledger.foundDefWire
        s
      case DefRegister(info, name, tpe, clock, reset, init) =>
        ledger.foundDefRegister
        s        
      case other => other
    }
